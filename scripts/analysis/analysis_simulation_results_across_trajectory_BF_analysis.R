...
source("./scripts/analysis/calculate_prestudy_odds_Carneiro.R")
...
# BF_analysis_data_equiv$sampsize_approach <- "standard"

# BF_data <- bind_rows(BF_analysis_data_sig, BF_analysis_data_equiv)

BF_data <- BF_analysis_data_sig

BF_data$trajectory <- interaction(BF_data$decision_crit, BF_data$sampsize_approach)


BF_data$H0 <- ifelse(BF_data$BF >= 10, 2, 1)

### add columns for prevalence and positives and negatives

BF_data$prev_pop      <- mat[1, 2]
BF_data$all_positives <- mat[2, 2]
BF_data$all_negatives <- mat[3, 2]
...
write.csv(outcomes,
          file = "./data/Carneiro_distribution/Bayes_factor_analysis/outcomes_sig_p0.05_method1_1.0_BF10")
