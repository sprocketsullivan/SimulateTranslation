...
outcomes <- 
  outcomes %>% 
  select(init_sample_size, SESOI, decision_crit, sampsize_approach,
...
ggplot(data = outcomes,
       aes(x = sampsize_approach, y = ES_detected,
           color = mean_N)) +
  geom_point(size = 3.5) +
  facet_grid(SESOI ~ decision_crit) +
  ggtitle("Number of detected true effects for each trajectory") +
  labs(x = "Approach to determine sample size",
       y = "# of experiments in which \ntrue in which true effect was detected", 
       color = "Mean # of \nanimals") +
